...
from utils import check_make_dir, generate_model_and_optimizer, load_model_and_optimizer, save_model_and_optimizer, \
    load_model_and_optimizer, negative_SDR, calculate_chunk_size, save_outputs
...
def main(dataset_dir: str, log_dir: str, train: bool, custom_test_dir: Optional[str], train_checkpoint_dir: str,
         test_output_dir: str,
         model: Optional[str], epoch_count: int, learning_rate: float, block_count: int, dropout: bool,
         dropout_proba: float, scale_pow: float):
    ...
    if model:
        ...
        if block_count < 1:
            raise Exception("Error: block count must be at least 1")
        if learning_rate <= 0:
            raise Exception("Error: learning rate must be above 0")
        if dropout and (dropout_proba < 0 or dropout_proba > 1):
            raise Exception("Error: dropout must be 0-1 inclusive")
        if scale_pow <= 0:
            raise Exception("Error: input scaling power must be above 0")

        # Create new model and optimizer from scratch
        audio_model, optimizer = generate_model_and_optimizer(block_count, dropout, dropout_proba, scale_pow,
    ...
    train_dataset, test_dataset = random_split(audio_dataset, [train_len, len(audio_dataset) - train_len],
                                               generator=torch.Generator().manual_seed(100)) if train else (
        None, audio_dataset)

    # Get DataLoader objects
    train_dataloader = DataLoader(train_dataset, shuffle=True) if train_dataset is not None else None
    ...
    for current_epoch in range(elapsed_epoch if train else 0, epoch_count if train else 1):
        ...
        if train:
            ...
            save_model_and_optimizer(audio_model, optimizer, block_count, learning_rate, dropout_proba, dropout,
                                     scale_pow, current_epoch + 1,
                                     os.path.join(model_checkpoint_dir,
                                                  "epoch_{}-{}".format(current_epoch + 1, time_str)))
        ...
        with alive_bar(len(test_dataloader)) as bar:
            ...
            for idx, i in enumerate(test_dataloader):
                ...
                with torch.no_grad():
                    ...
                    if not train: save_outputs(mixture_chunk_list, pred_chunk_list_dict, target_chunk_list_dict,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--train-checkpoint-dir", metavar="[root directory to store checkpoints]",
                        help="Root directory to store checkpoints of model during training (default: ./checkpoints)",
                        default="./checkpoints")
    ...
    parser.add_argument("--dropout-proba", metavar="[dropout probability]",
                        help="Probability used for dropout layers; ignored if --dropout is not used as well " +
                             "(default: 0.2)",
                        default=0.2)
    parser.add_argument("--scale-pow", metavar="[power to scale inputs by]",
                        help="Exponent to scale input samples by, while preserving amplitude signs (default: 0.5)",
                        default=0.5)

    # Parse arguments and call main function
    args = parser.parse_args()
    main(args.dataset_dir, args.log_dir, not args.test, args.custom_test_dir, args.train_checkpoint_dir,
         args.test_output_dir, args.model, int(args.epoch_count),
         float(args.learning_rate), int(args.block_count), args.dropout, float(args.dropout_proba),
         float(args.scale_pow))
