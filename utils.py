...
def save_model_and_optimizer(model: nn.DataParallel, optimizer: torch.optim, block_count: int, learning_rate: float,
                             dropout_proba: float,
...
def load_model_and_optimizer(device: torch.device, path: str):
    ...
    model, optimizer = generate_model_and_optimizer(loaded_data['block_count'], loaded_data['dropout'],
                                                    loaded_data['dropout_proba'], loaded_data['scale_pow'],
                                                    loaded_data['learning_rate'], device)
...
def generate_model_and_optimizer(block_count: int, dropout: bool, dropout_proba: float, scale_pow: float,
    ...
    audio_model = nn.DataParallel(UNet(feature_count_list, KERNEL_SIZE, "gelu", INSTRUMENTS,
                                       ...
                                       dropout=dropout, dropout_proba=dropout_proba,
...
def save_outputs(mixture_chunks_list: typing.List[torch.Tensor],
                 pred_waveform_list_dict: typing.Dict[str, typing.List[torch.Tensor]],
    ...
    try:
        ...
        torchaudio.save(os.path.join(path, "mixture.wav"), torch.cat(mixture_chunks_list, dim=-1).reshape((2, -1)),
                        SAMPLING_RATE)

        # Save separated components (predicted and actual)
        for i in INSTRUMENTS:
            torchaudio.save(os.path.join(path, "{}_pred.wav".format(i)),
                            torch.cat(pred_waveform_list_dict[i], dim=-1).reshape((2, -1)), SAMPLING_RATE, format="wav")
            torchaudio.save(os.path.join(path, "{}.wav".format(i)),
                            torch.cat(target_waveform_list_dict[i], dim=-1).reshape((2, -1)), SAMPLING_RATE,
                            format="wav")
    except Exception as e:
        raise Exception("Error: \"{}\".\nYou may have run out of space.".format(e))


def save_attacks(attack_chunks: typing.Dict[float, typing.List[torch.Tensor]],
                 pred_waveforms: typing.Dict[float, typing.Dict[str, typing.List[torch.Tensor]]],
                 target_waveforms: typing.Dict[str, typing.List[torch.Tensor]],
                 path: str):
    # Create directory
    check_make_dir(path)
    ...
    try:
        # Save original mixture wav
        for eta, chunks in attack_chunks.items():
            torchaudio.save(os.path.join(path, f"attack_{eta}.wav"), torch.cat(chunks, dim=-1).reshape((2, -1)),
                            SAMPLING_RATE)

        # Save separated components (predicted and actual)
        for i in INSTRUMENTS:
            for eta, waveforms in pred_waveforms.items():
                torchaudio.save(os.path.join(path, f"{i}_pred_{eta}.wav"),
                                torch.cat(waveforms[i], dim=-1).reshape((2, -1)), SAMPLING_RATE, format="wav")
            torchaudio.save(os.path.join(path, "{}.wav".format(i)),
                            torch.cat(target_waveforms[i], dim=-1).reshape((2, -1)), SAMPLING_RATE,
                            format="wav")
    except Exception as e:
        raise Exception("Error: \"{}\".\nYou may have run out of space.".format(e))
